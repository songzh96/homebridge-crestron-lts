...
export class LightAccessory {
  ...
  constructor(
    ...
    this.eventEmitter.on(`${this.deviceType}:${this.id}:${this.eventPowerStateMsg}`, this.eventLightStateMsgEvent.bind(this));
  ...
  async setOn(value: CharacteristicValue) {
    ...
    if (this.States.On !== tmpValue) {
      ...
      this.platform.log.debug(`${this.deviceType}:${this.id}: Set Characteristic On By Homekit -> ${this.States.On}`);
    }
  }
  ...
  async getOn(): Promise<CharacteristicValue> {
    ...
    this.platform.log.debug(`${this.deviceType}:${this.id}: Get Characteristic On From Homekit -> ${isOn}`);
    this.platform.sendData(`${this.deviceType}:${this.id}:${this.getPowerStateMsg}:*`);
    return isOn;
  }

  async eventLightStateMsgEvent(value: number) {
    const tmpValue = (value === 1) ? true : false;

    if (this.States.On !== tmpValue) {
      this.States.On = tmpValue;
      this.platform.log.debug(`${this.deviceType}:${this.id}: Event Characteristic On By Crestron Processor -> ${this.States.On}`);
